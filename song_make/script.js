...
async function repeatTwice() {
  ...
  const addtime=nextSample.notes[nextSample.notes.length-1].quantizedEndStep;
...
function updateUI(state) {
  ...
  switch(state) {
    case 'model-loading':
      ...
      show('status');
      show('loadingStatus');
      document.getElementById('loadingStatus').textContent = '正在载入示例样本...';
      document.getElementById('status').textContent = '正在载入模型...请耐心等待...';
      disable('btnSample');
      hide('changeSample');
      break;
    case 'model-loaded':
    ...
    case 'file-loading':
      ...
      hide('input');
      hide('hasMelody');
      document.getElementById('status').textContent = '正在载入你选择的Midi音乐...';
      break;
    case 'file-error':
      show('status');
      document.getElementById('status').textContent = '您的Midi文件载入失败，请核对后重试...';
      break;
    case 'file-loaded':
      hide('status');
      show('input');
